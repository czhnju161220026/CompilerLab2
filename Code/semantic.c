...
bool handleFunDec(Morpheme* root, Symbol* s) {
    ...
    if (c->type == _ID && c->siblings != NULL && c->siblings->type == _LP && c->siblings->siblings != NULL 
        && c->siblings->siblings->type == _VarList && c->siblings->siblings->siblings != NULL 
        && c->siblings->siblings->siblings->type == _RP) {
        if (c->idName != NULL) {
	    setSymbolName(s, c->idName);
...
bool handleVarList(Morpheme* root, Symbol* s) {
    ...
    if (c->type == _ParamDec && c->siblings != NULL && c->siblings->type == _COMMA && c->siblings->siblings != NULL
        && c->siblings->siblings->type == _VarList) {
